...
const rivelleSchema = z.object({
  ...
  sourceRoot: z.string(),
  style: z.enum(["nova", "prism"]).default("nova"),
  theme: z.object({
    font: z.enum(["geist", "inter"]),
    neutral: z.string(),
    accent: z.string(),
    radius: z.string(),
  }),
...
type InitAnswers = {
  ...
  install: boolean;
  sourceRoot: string;
  framework: RivelleConfig["framework"];
};
...
program
  ...
  .option("--style <style>", "component style: nova or prism")
  ...
  .action(async (options) => {
    ...
    if (existsSync(configPath) && !options.force)
      fail("components.json already exists. Use --force to replace it.");
    ...
    const config: Config = configSchema.parse({
      ...
      aliases,
      registries: {
        "@rivelle": `${options.registry.replace(/\/$/, "")}/{name}.json`,
      },
    });
    const rivelle: RivelleConfig = {
      version: 1,
    ...
    await writeText(
      join(cwd, "rivelle.json"),
      `${JSON.stringify(rivelle, null, 2)}\n`,
      true,
    );
    ...
    await writeText(
      resolveWithin(
        cwd,
        answers.sourceRoot,
        `lib/utils.${answers.tsx ? "ts" : "js"}`,
      ),
      answers.tsx ? utilsSource : utilsSourceJs,
      options.force,
    );
    ...
    if (!options.yes && process.stdin.isTTY)
      p.outro("Rivelle is ready. Try: rivelle add button");
...
program
  ...
  .action(async (components: string[], options) => {
    ...
    const packages = new Set<string>(
      options.skipInstall
        ? []
        : await missingDependencies(cwd, themeDependencies(rivelle.theme.font)),
    );
    for (const component of components)
      await addItem(component, {
        cwd,
        config,
        rivelle,
    ...
    if (!options.skipInstall && packages.size > 0)
      await install(cwd, [...packages]);
...
async function detectProject(cwd: string) {
  const packageJson = (await readJsonIfExists(join(cwd, "package.json"))) as
    | {
  ...
  const framework: RivelleConfig["framework"] = dependencies.next
    ? "next"
    : dependencies.vite
      ? "vite"
      : "react";
  ...
  const css =
    ...
    (framework === "next"
      ? joinSlash(sourceRoot, "app/globals.css")
      : joinSlash(sourceRoot, "index.css"));
  return {
    framework,
    sourceRoot,
    css,
    tsx: existsSync(join(cwd, "tsconfig.json")),
    rsc: framework === "next",
  };
}
...
async function getInitAnswers(
  ...
  const defaults = validateInitAnswers({
    css: options.css ?? detected.css,
    style: options.style ?? "nova",
    font: options.font ?? "geist",
    baseColor: options.baseColor ?? "neutral",
    accent: options.accent ?? "indigo",
    radius: options.radius ?? "0.625rem",
    rsc: options.rsc !== false && detected.rsc,
    tsx: options.javascript ? false : detected.tsx,
  ...
  const result = await p.group(
    {
      style: () =>
        p.select({
          message: "Which component style should Rivelle use?",
          initialValue: defaults.style,
          options: [
            { value: "nova", label: "Nova", hint: "signature Rivelle" },
            { value: "prism", label: "Prism", hint: "luminous and precise" },
          ],
        }),
      font: () =>
        p.select({
          message: "Which font should Rivelle use?",
          initialValue: defaults.font,
          options: [
            { value: "geist", label: "Geist", hint: "recommended" },
            { value: "inter", label: "Inter" },
          ],
        }),
      baseColor: () =>
        p.select({
          message: "Choose a neutral palette",
          initialValue: defaults.baseColor,
          options: Object.keys(neutralPalettes).map(option),
        }),
      accent: () =>
        p.select({
          message: "Choose an accent color",
          initialValue: defaults.accent,
          options: Object.keys(accentPalettes).map(option),
        }),
      radius: () =>
        p.select({
          message: "Choose the base radius",
          initialValue: defaults.radius,
          options: ["0rem", "0.5rem", "0.625rem", "0.75rem", "1rem"].map(
      ...
      css: () =>
        p.text({
          ...
          initialValue: defaults.css,
        }),
      tsx: () =>
        p.confirm({ message: "Use TypeScript?", initialValue: defaults.tsx }),
      rsc: () =>
        p.confirm({
          message: "Use React Server Components?",
          initialValue: defaults.rsc,
        }),
      install: () =>
        p.confirm({
...
function validateInitAnswers(answers: InitAnswers) {
  if (answers.style !== "nova" && answers.style !== "prism")
    throw new Error("Style must be either nova or prism.");
  if (answers.font !== "geist" && answers.font !== "inter")
    throw new Error("Font must be either geist or inter.");
  if (!isNeutralName(answers.baseColor))
    throw new Error("Unsupported base color.");
  if (!isAccentName(answers.accent))
    throw new Error("Unsupported accent color.");
  if (!/^\d+(\.\d+)?(rem|px)$/.test(answers.radius))
    throw new Error("Radius must use rem or px units.");
  return answers;
}
...
async function addItem(
  ...
  try {
    ...
    for (const dependency of item.registryDependencies)
      await addItem(
        dependency.startsWith(`${DEFAULT_REGISTRY}/`)
          ? dependency.replace(DEFAULT_REGISTRY, registryBase(context.config))
          : dependency,
        context,
      );
    for (const dependency of item.dependencies)
      context.packages.add(dependency);

    for (const file of item.files) {
      const target = resolveTarget(file, context);
      let content = file.content;
      if (!context.config.rsc)
        content = content.replace(/^"use client"\s*\n+/, "");
      if (!context.config.tsx && /\.(tsx?|mts|cts)$/.test(file.path))
        content = toJavaScript(content, file.path.endsWith("x"));
      ...
      if (written)
        console.log(`${pc.green("+")} ${relative(context.cwd, target)}`);
    }
...
function resolveTarget(
  ...
  context: { cwd: string; config: Config; rivelle: RivelleConfig },
) {
  const extension = context.config.tsx
    ? extname(file.path)
    : extname(file.path).replace("ts", "js");
  const filename = `${basename(file.path, extname(file.path))}${extension}`;
  if (file.target?.startsWith("~/"))
    return resolveWithin(context.cwd, file.target.slice(2));
...
async function readConfig(cwd: string) {
  ...
  if (!existsSync(path))
    throw new Error("components.json was not found. Run `rivelle init` first.");
  return configSchema.parse(JSON.parse(await readFile(path, "utf8")));
}

async function readRivelleConfig(
  cwd: string,
  config: Config,
): Promise<RivelleConfig> {
  const path = join(cwd, "rivelle.json");
  if (existsSync(path))
    return rivelleSchema.parse(JSON.parse(await readFile(path, "utf8")));
  const sourceRoot = config.tailwind.css.startsWith("src/") ? "src" : "";
  return {
    version: 1,
    framework: config.rsc ? "next" : "react",
    sourceRoot,
    style: config.style === "prism" ? "prism" : "nova",
    theme: {
      font: "geist",
      neutral: config.tailwind.baseColor,
      accent: "indigo",
      radius: "0.625rem",
    },
  };
...
async function mergeTheme(path: string, answers: InitAnswers) {
  let current = existsSync(path)
    ? await readFile(path, "utf8")
    : '@import "tailwindcss";\n';
  current = current
    .replace(/^@import "@fontsource-variable\/(geist|inter)";\s*\r?\n?/gm, "")
    .replace(/^@import "tw-animate-css";\s*\r?\n?/gm, "");
  ...
  const pattern = new RegExp(
    `${escapeRegex(THEME_START)}[\\s\\S]*?${escapeRegex(THEME_END)}`,
    "m",
  );
  current = pattern.test(current)
    ? current.replace(pattern, theme)
    : `${current.trim()}\n\n${theme}`;
  await mkdir(dirname(path), { recursive: true });
...
async function ensureViteAlias(cwd: string, sourceRoot: string) {
  ...
  if (!/defineConfig\s*\(\s*\{/.test(source)) {
    console.warn(
      pc.yellow(
        `Could not safely add the @ alias to ${existingName}. Configure it to point to ${aliasTarget}.`,
      ),
    );
    return;
  }
  if (
    ...
    source = `import { fileURLToPath, URL } from "node:url"\n${source}`;
...
async function install(cwd: string, dependencies: string[]) {
  ...
  console.log(pc.dim(`${manager} ${args.join(" ")}`));
...
function printInstallHint(cwd: string, dependencies: string[]) {
  ...
  console.warn(
    pc.yellow(
      `Dependencies were not installed. Run: ${manager} ${verb} ${dependencies.join(" ")}`,
    ),
  );
}
...
function resolveItemUrl(name: string, config: Config) {
  ...
  const [namespace, item] = name.startsWith("@")
    ? name.split("/", 2)
    : ["@rivelle", name];
  const template = config.registries[namespace];
  if (!template)
    throw new Error(
      `Registry ${namespace} is not configured in components.json.`,
    );
  return template.replace("{name}", item);
}
function registryBase(config: Config) {
  return config.registries["@rivelle"].replace(/\/{name}\.json$/, "");
}
function joinSlash(...parts: string[]) {
...
function createThemeSource({
  ...
  return `${THEME_START}\n/* --rivelle-theme: generated by rivelle init. */\n@custom-variant dark (&:is(.dark *));\n\n${createThemeVariablesCss({ style, font, neutral: baseColor, accent, radius })}\n\n${themeBridgeSource}\n${THEME_END}`;
}

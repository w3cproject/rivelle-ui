...
const defaults: RuntimeTheme = {
  ...defaultTheme,
  style: defaultTheme.style ?? "nova",
  mode: "dark",
};
const ThemeContext = React.createContext<{
  settings: RuntimeTheme;
  update: <K extends keyof RuntimeTheme>(
    key: K,
    value: RuntimeTheme[K],
  ) => void;
  reset: () => void;
} | null>(null);

export function RivelleThemeProvider({
  children,
}: {
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      setSettings({
        style:
          value.style && isStyleName(value.style)
            ? value.style
            : defaults.style,
        neutral:
          value.neutral && isNeutralName(value.neutral)
            ? value.neutral
            : defaults.neutral,
        accent:
          value.accent && isAccentName(value.accent)
            ? value.accent
            : defaults.accent,
        font: value.font === "inter" ? "inter" : "geist",
        radius:
          typeof value.radius === "string" ? value.radius : defaults.radius,
        mode:
          storedMode === "light" || storedMode === "dark"
            ? storedMode
            : value.mode === "light"
  ...
  const value = React.useMemo(
    () => ({
      settings,
      update: <K extends keyof RuntimeTheme>(key: K, next: RuntimeTheme[K]) =>
...
export function useRivelleTheme() {
  ...
  if (!context)
    throw new Error("useRivelleTheme must be used inside RivelleThemeProvider");
  return context;
}
...
function applyTheme(settings: RuntimeTheme) {
  ...
  Object.entries(createThemeTokens(settings, settings.mode === "dark")).forEach(

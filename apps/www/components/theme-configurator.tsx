...
export function ThemeConfigurator() {
  ...
  const initCommand = `pnpm dlx rivelle@latest init --style ${settings.style} --font ${settings.font} --base-color ${settings.neutral} --accent ${settings.accent} --radius ${settings.radius}`;
    ...
    <div className="theme-studio">
      <div className="theme-controls">
        <div className="theme-control-heading">
          <div>
            ...
            <strong>Shape the foundations</strong>
          </div>
          <button aria-label="Reset theme" onClick={reset} type="button">
            <RotateCcw />
          </button>
        </div>

        <ThemeGroup label="Component style">
          <div className="theme-segmented">
            {(["nova", "prism"] as StyleName[]).map((style) => (
              <button
        ...
        <ThemeGroup label="Neutral palette">
          <div className="theme-swatch-grid">
            {(Object.keys(neutrals) as NeutralName[]).map((name) => (
              <button
                className={cn(settings.neutral === name && "is-active")}
                key={name}
                onClick={() => update("neutral", name)}
                type="button"
              >
                <i style={{ background: neutralSwatch(name) }} />
                {name}
                <Check />
              </button>
            ))}
        ...
        <ThemeGroup label="Accent">
          <div className="theme-accent-row">
            {(Object.keys(accents) as AccentName[]).map((name) => (
              <button
                aria-label={name}
                className={cn(settings.accent === name && "is-active")}
                key={name}
        ...
        <ThemeGroup label="Font">
          <div className="theme-segmented">
            {(["geist", "inter"] as FontName[]).map((font) => (
              <button
                className={cn(settings.font === font && "is-active")}
                key={font}
        ...
        <ThemeGroup label="Radius">
          <div className="theme-radius-row">
            {radii.map((radius) => (
              <button
                className={cn(settings.radius === radius && "is-active")}
                key={radius}
                onClick={() => update("radius", radius)}
                type="button"
              >
                <i style={{ borderRadius: radius }} />
                {radius === "0rem" ? "0" : radius.replace("rem", "")}
              </button>
            ))}
          </div>
        </ThemeGroup>
        ...
        <ThemeGroup label="Appearance">
          <div className="theme-segmented">
            <button
              className={cn(settings.mode === "light" && "is-active")}
            ...
            <button
              className={cn(settings.mode === "dark" && "is-active")}
      ...
      <div className="theme-preview" id="preview">
        <div className="theme-preview-bar">
          ...
          <small>
            {settings.style} · {settings.neutral} · {settings.accent} ·{" "}
            {settings.font}
          </small>
        </div>
          ...
          <Card className="w-full max-w-[390px] shadow-xl shadow-black/5">
            <CardHeader>
              <div className="mb-3 flex items-center justify-between">
                ...
                <Switch aria-label="Enable project" defaultChecked />
              </div>
              <CardTitle>Launch with clarity.</CardTitle>
              <CardDescription>
                A compact preview using the same tokens as every Rivelle
                component.
              </CardDescription>
            </CardHeader>
      ...
      <div className="theme-code" id="css-variables">
        <div>
          ...
          <span className="theme-code-actions">
            <button onClick={copyCommand} type="button">
              {copiedCommand ? <Check /> : <Copy />}
              {copiedCommand ? "Copied" : "Copy init"}
            </button>
        ...
        <div className="theme-command-line">
          <span>$</span>
          {initCommand}
        </div>
        <pre>
...
function neutralSwatch(name: NeutralName) {
  const item = neutrals[name];
  return `oklch(.62 ${item.chroma * 2} ${item.hue})`;
}
function accentSwatch(name: AccentName) {
  const item = accents[name];
  return `oklch(.62 ${item.chroma} ${item.hue})`;
}
